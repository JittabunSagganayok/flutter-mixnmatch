...
class DioProvider {
  ...
  Future<dynamic> getToken(String email, String password) async {
    try {
      var response = await Dio().post('${baseApi}api/login',
  ...
  Future<dynamic> getUser(String token) async {
    try {
      var user = await Dio().get('${baseApi}api/user',
  ...
  Future<dynamic> bookAppointment(
    ...
    try {
      var response = await Dio().post('${baseApi}api/book',
  ...
  Future<dynamic> getAppointments(String token) async {
    try {
      var response = await Dio().get('${baseApi}api/appointments',
          options: Options(headers: {'Authorization': 'Bearer $token'}));
  ...
  Future<dynamic> storeReviews(
    ...
    try {
      var response = await Dio().post('${baseApi}api/reviews',
          data: {
            'ratings': ratings,
            'reviews': reviews,
            'appointment_id': id,
  ...
  Future<dynamic> storeFavDoc(String token, List<dynamic> favList) async {
    try {
      var response = await Dio().post('${baseApi}api/fav',
          data: {
            'favList': favList,
          },
          options: Options(headers: {'Authorization': 'Bearer $token'}));
...
//logout
  Future<dynamic> logout(String token) async {
    try {
      var response = await Dio().post('${baseApi}api/logout',
          options: Options(headers: {'Authorization': 'Bearer $token'}));
